...
- `{env}`: Environment name (e.g., 'local', 'test', 'dev', 'prod') determined by the `APP_ENV` environment variable (defaults to 'local')

The orchestrator will look for model configurations in the following directories (in order):
1. `config/local/models/` (for local development)
2. `config/models/{env}/` (alternative structure)
3. `config/{env}/models/` (preferred structure)
4. `config/models/local/` (legacy structure)

If none of these directories exist, the orchestrator will create `config/{env}/models/`.
