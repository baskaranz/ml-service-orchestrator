...
class PlatformConfig(BaseModel):
    """Platform configuration for a model."""
...
class BasicErrorHandlingConfig(BaseModel):
    ...
    max_retry_delay: float = Field(default=30.0, ge=0.0, description="Maximum retry delay in seconds")
...
class LLMErrorHandlingConfig(BaseModel):
    ...
    provider: Optional[LLMProviderConfig] = Field(default=None, description="LLM provider configuration")
...
class BasicCircuitBreakerConfig(BaseModel):
    ...
    reset_timeout: float = Field(default=60.0, ge=1.0, description="Seconds to wait before attempting to close circuit")
    half_open_timeout: float = Field(default=30.0, ge=1.0, description="Seconds in half-open state")
    success_threshold: int = Field(default=2, ge=1, description="Successful requests to close circuit")
...
class HealthCheckConfig(BaseModel):
    ...
    failure_threshold: int = Field(default=3, ge=1, description="Number of failures before marking unhealthy")
    success_threshold: int = Field(default=2, ge=1, description="Number of successes before marking healthy")
...
class ModelConfig(BaseModel):
    ...
    http2: bool = Field(
        default=True,
        description="Whether to enable HTTP/2 support. Defaults to True for better performance with HTTP/2 capable servers."
    )
    ...
    def validate_platform(cls, v: Any) -> Optional[Union[PlatformConfig, Dict[str, Any]]]:
        ...
        if isinstance(v, str):
            try:
                ...
                raise ValueError("Platform config must be a valid JSON string, dict, or PlatformConfig object")
...
class ModelRegistry(BaseModel):
    ...
    description: str = Field(default="Registry of model configurations", description="Description of the registry")
