...
def test_model_config_validation():
    ...
    config = ModelConfig(
        ...
        circuit_breaker=CircuitBreakerConfig(
            basic=BasicCircuitBreakerConfig(failure_threshold=5, reset_timeout=30.0)
        ),
    )
    ...
    config2 = ModelConfig(
        ...
        active=False,
        circuit_breaker=CircuitBreakerConfig(
            basic=BasicCircuitBreakerConfig(failure_threshold=3, reset_timeout=15.0)
        ),
    )
    ...
    with pytest.raises(ValidationError):
        BasicCircuitBreakerConfig(failure_threshold=-1, reset_timeout=30.0)
    with pytest.raises(ValidationError):
        BasicCircuitBreakerConfig(failure_threshold=5, reset_timeout=-1.0)
...
def test_auth_config_validation():
    ...
    api_key_auth = AuthConfig(
        type=AuthType.API_KEY, header_name="X-API-Key", value="test-key", enabled=True
    )
...
def test_cache_config_validation():
    ...
    minimal_cache = CacheConfig(enabled=False, ttl=300, max_size=100, vary_by_headers=[])
    ...
    custom_cache = CacheConfig(
        enabled=True, ttl=600, max_size=200, vary_by_headers=["Content-Type", "Accept"]
    )
...
def test_required_arguments_validation():
    ...
    ModelConfig(id="test_model_3", name="Test Model", endpoint_url="http://localhost:8000/predict")

    # Now test missing id
    with pytest.raises(ValidationError) as exc_info:
        ModelConfig(name="Test Model", endpoint_url="http://localhost:8000/predict")
    assert "id" in str(exc_info.value)

    # Now test missing endpoint_url
    with pytest.raises(ValidationError) as exc_info:
        ModelConfig(id="test_model_4", name="Test Model")
