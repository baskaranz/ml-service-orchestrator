...
def test_admin_list_models(MockModelRegistry, client: TestClient, setup_api_key):
    ...
    mock_models = [
        ModelConfig(
            id="test_model_1", name="Test Model 1", endpoint_url="http://test1.com", active=True
        ),
        ModelConfig(
            id="test_model_2", name="Test Model 2", endpoint_url="http://test2.com", active=True
        ),
    ]
...
def test_admin_get_model(
    mock_get_model, client: TestClient, setup_api_key, model_config_instance, llm_provider_config
):
...
def test_admin_add_model(
    mock_add_model, client: TestClient, setup_api_key, model_config_instance, llm_provider_config
):
...
@patch("app.services.model_registry.ModelRegistryService.add_model", new_callable=AsyncMock)
def test_admin_add_model_conflict(
    mock_add_model, client: TestClient, setup_api_key, model_config_instance, llm_provider_config
):
...
def test_admin_update_model(
    mock_update_model, client: TestClient, setup_api_key, model_config_instance, llm_provider_config
):
    """Test updating an existing model."""
    updated_model = model_config_instance.model_copy(
        update={"name": "Updated Model", "description": "This model was updated"}
    )
...
@patch("app.services.model_registry.ModelRegistryService.update_model", new_callable=AsyncMock)
def test_admin_update_model_id_mismatch(
    mock_update_model, client: TestClient, setup_api_key, model_config_instance, llm_provider_config
):
...
@patch("app.services.model_registry.ModelRegistryService.update_model", new_callable=AsyncMock)
def test_admin_update_model_not_found(
    mock_update_model, client: TestClient, setup_api_key, model_config_instance, llm_provider_config
):
