...
def cleanup_docker_services() -> bool:
    ...
    success, output = run_command(["docker", "ps", "-a", "--filter", "name=mock-", "--format", "{{.Names}}"])
    if not success:
    ...
    success, output = run_command(["docker", "network", "ls", "--filter", "name=mock", "--format", "{{.Name}}"])
    ...
    for compose_file in compose_files:
        if compose_file.exists():
            ...
            run_command(["docker-compose", "-f", str(compose_file), "down", "--remove-orphans"], 
