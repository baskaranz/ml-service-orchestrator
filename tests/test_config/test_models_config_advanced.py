    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("app.config.models_config.logger.error") as mock_error:
            try:
                ...
                assert (
                    "valid_model" in models
                ), f"valid_model not found in loaded models: {list(models.keys())}"
                ...
                error_logged = any(
                    "Validation error creating ModelConfig" in str(call)
...
async def test_update_model_config_not_found(llm_provider_config):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        model = ModelConfig(
            ...
            timeout=30.0,
            max_retries=3,
            circuit_breaker=CircuitBreakerConfig(failure_threshold=5, reset_timeout=30.0),
