...
async def get_orchestrator() -> Orchestrator:
    ...
    async with _orchestrator_lock:
        ...
        try:
            ...
            _orchestrator = Orchestrator(config_dir=str(settings.models_dir_path))
...
async def forward_request(
    ...
    try:
        ...
        response = await orchestrator.proxy_request(
            model_config=model_config, request=request, path_suffix="/predict"
        )
...
async def get_model_health(
    model_id: str, orchestrator: Orchestrator = Depends(get_orchestrator)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting health for model '{model_id}': {str(e)}"
        )
...
async def refresh_models(orchestrator: Orchestrator = Depends(get_orchestrator)) -> Dict[str, Any]:
    """
