...
class HttpClient:
    ...
    async def request(
        ...
        logger.debug(
            f"Content length: {len(content) if content else 0} bytes" if content else "No content"
        )
        ...
        while attempt < self.max_retries:
            try:
                ...
                if (
                    ...
                    and isinstance(response, MagicMock)
                ):
                ...
                delay = (
                    self.backoff_factor * (2 ** (attempt - 1)) if attempt == 1 else self.retry_delay
                )
...
def build_url(
    base_url: str, path: Optional[str] = None, query_params: Optional[Dict[str, Any]] = None
) -> str:
...
def get_auth_params(auth_config: Optional[AuthConfig] = None) -> Dict[str, str]:
    ...
    if (
        not auth_config
        or not auth_config.enabled
        or auth_config.type != AuthType.API_KEY
        or auth_config.location != AuthLocation.QUERY
        or not auth_config.key_name
        or not auth_config.key_value
    ):
        return {}
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout) as client:
            response = await client.request(
                method=method, url=url, headers=headers, json=json, content=content, params=params
            )
