...
class LLMCircuitBreakerConfig:
    ...
    error_thresholds: Dict[LLMErrorType, int] = field(
...
class LLMCircuitBreaker(CircuitBreaker, Generic[T]):
    ...
    def __init__(self, llm: Any, config: LLMCircuitBreakerConfig):
        ...
        self.llm = llm
        self.config = config
        self._error_counts: Dict[LLMErrorType, int] = {error_type: 0 for error_type in LLMErrorType}
    ...
    def execute(self, prompt: Any, *args: Any, **kwargs: Any) -> T:
        ...
        while retries <= self.config.max_retries:
            try:
                logger.debug(
                    f"Attempting LLM call (attempt {retries + 1}/{self.config.max_retries + 1})"
                )
    ...
    async def execute_async(self, prompt: Any, *args: Any, **kwargs: Any) -> T:
        ...
        while retries <= self.config.max_retries:
            try:
                logger.debug(
                    f"Attempting async LLM call (attempt {retries + 1}/{self.config.max_retries + 1})"
                )
