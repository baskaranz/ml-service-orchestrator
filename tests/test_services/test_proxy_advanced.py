...
async def test_proxy_service_with_path_suffix(mock_request):
    ...
    mock_orchestrator.proxy_request.assert_called_with(
        model_config=model_config, request=mock_request, path_suffix=""
    )
    ...
    mock_orchestrator.proxy_request.assert_called_with(
        model_config=model_config, request=mock_request, path_suffix="predict"
    )
    ...
    mock_orchestrator.proxy_request.assert_called_with(
        model_config=model_config, request=mock_request, path_suffix="v2/predict/image"
    )
...
async def test_proxy_service_with_advanced_request(advanced_request):
    ...
    response = await service.proxy_to_model("test-model", advanced_request, "classify")
