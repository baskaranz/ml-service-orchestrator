...
class ModelConfigManager:
    ...
    def __init__(self, config_dir: str = None, env: str = None):
        ...
        if self.env not in valid_envs:
            raise ValueError(f"Invalid environment: {self.env}. Must be one of: {', '.join(valid_envs)}")
        ...
        logger.info(f"Current working directory: {os.getcwd()}")
        logger.info(f"Environment variables: { {k: v for k, v in os.environ.items() if k in ('APP_ENV', 'MODELS_DIR', 'CONFIG_DIR', 'CONFIG_PATH')} }")
    ...
    async def load_configs(self) -> Tuple[ModelRegistry, Dict[str, ModelConfig]]:
        ...
        try:
            ...
            logger.info(f"Found {len(yaml_files)} YAML files in {models_path}: {[f.name for f in yaml_files]}")
            ...
            for file_path in yaml_files:
                ...
                try:
                    ...
                    if not model_dict or not isinstance(model_dict, dict):
                        logger.warning(f"Skipping invalid model file (not a YAML dictionary): {file_path}")
                        continue
                    ...
                    logger.info(f"Model {model_dict.get('id')} active status: {model_dict.get('active')} (type: {type(model_dict.get('active'))})")
                    ...
                    try:
                        ...
                        logger.info(f"Successfully created ModelConfig for '{model_config.id}' with active={active_status}")
                        ...
                        logger.info(f"Successfully loaded model configuration for '{model_config.id}'")
                    except ValidationError as e:
                        logger.error(f"Validation error creating ModelConfig from {file_path}: {str(e)}")
                        continue
                except ValidationError as e:
    ...
    def _process_environment_vars(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in config.items():
            if isinstance(value, str) and value.startswith('${') and value.endswith('}'):
                ...
                processed[key] = [
                    self._process_environment_vars(v) if isinstance(v, dict) else 
                    (os.getenv(v[2:-1], v) if isinstance(v, str) and v.startswith('${') and v.endswith('}') else v)
                    for v in value
                ]
            else:
                processed[key] = value
