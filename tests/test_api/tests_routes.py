...
def test_orchestrator_endpoint(mock_proxy, client: TestClient):
    ...
    mock_response = Response(
        content=json.dumps({"result": "success"}), status_code=200, media_type="application/json"
    )
...
def test_admin_list_models(mock_list_models, client: TestClient):
    ...
    model_1 = ModelSummary(
        id="model_1", name="Model 1", description="desc", version="1.0.0", active=True
    )
    model_2 = ModelSummary(
        id="model_2", name="Model 2", description="desc", version="1.0.0", active=False
    )
